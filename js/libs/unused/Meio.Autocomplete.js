...
(function(global, $) {
	...
	var keysThatDontChangeValueOnKeyUp = {
		...
		// alt
		224: 1,
		// command (meta onkeypress)
		91: 1,
		// command (meta onkeydown)
		37: 1,
		// left
		38: 1,
		// up
...
/*
		...
		update: function() {
			...
			if (cached) {
				...
				for (var row, i = 0, n = 0; row = data[i++];) if (filter.call(this, text, row)) {
					itemsHtml.push('<li title="', encode(formatMatch.call(this, text, row)), '" data-index="', n, '" class="', (n % 2 ? classes.even : classes.odd), '">', formatItem.call(this, text, row, n), '</li>');
					itemsData.push(row);
					n++;
				}
				html = itemsHtml.join('');
	...
	Meio.Autocomplete.Select = new Class({
		...
		options: {
			syncName: 'id',
			// if falsy it wont sync at start
			valueField: null,
	...
	Meio.Autocomplete.Select.One = new Class({
		...
		createDataArray: function() {
			...
			for (var i = 0, selectOption, optionValue; selectOption = selectOptions[i++];) {
				...
				if (optionValue || optionValue === 0) data.push({
					value: optionValue,
	...
	Meio.Element.List = new Class({
		...
		options: {
			width: 'field',
			// you can pass any other value settable by set('width') to the list container
			classes: {
		...
		render: function() {
			...
			if (node.bgiframe) node.bgiframe({
				top: 0,
				left: 0
			});
